...
jQuery(document).ready(function($) {
  ...
  $("#_customize-input-wedev_general_font").selectize();
    ...
    jQuery('#_customize-input-wedev_general_font').on('change',function(){
        ...
        var family = $(this).val();
        var ajaxurl = anecdote_lite_customizer.ajax_url;
        var data = {
            'action': 'anecdote_lite_fonts_ajax',
            'family': family,
        };
  ...
  $("#_customize-input-wedev_heading_font").selectize();
    ...
    jQuery('#_customize-input-wedev_heading_font').on('change',function(){
        ...
        $.post(ajaxurl, data, function( response ) {
            ...
            select = $('#_customize-input-anecdote_lite_h1_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h1_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );

            select = $('#_customize-input-anecdote_lite_h2_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h2_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );

            select = $('#_customize-input-anecdote_lite_h3_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h3_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );

            select = $('#_customize-input-anecdote_lite_h4_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h4_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );

            select = $('#_customize-input-anecdote_lite_h5_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h5_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );

            select = $('#_customize-input-anecdote_lite_h6_font_weight');
            select.empty().append(response);
             wp.customize( 'anecdote_lite_h6_font_weight', function ( obj ) {
                obj.set( 'regular' );
            } );
  ...
  $("#_customize-input-wedev_tagline_font").selectize();
    ...
    // Save Value.
    function anecdote_lite_refresh_repeater_values(){

        $(".anecdote-lite-repeater-field-control-wrap").each(function(){
            ...
            current.next('.anecdote-lite-repeater-collector').val( JSON.stringify( values ) ).trigger('change');
        });

    }

    $("body").on("click",'.anecdote-lite-add-control-field', function(){
        ...
        if(typeof current != 'undefined') {
            ...
            if(typeof field != 'undefined'){
                ...
                field.find('.anecdote-lite-fields').show();

                current.find('.anecdote-lite-repeater-field-control-wrap').append(field);
                $('.accordion-section-content').animate({ scrollTop: current.height() }, 1000);
                anecdote_lite_refresh_repeater_values();
            }

            $('.anecdote-lite-repeater-field-control-wrap li').removeClass('wedevs-sortable-active');
            $('.anecdote-lite-repeater-field-control-wrap li:last-child').addClass('wedevs-sortable-active');
            $('.anecdote-lite-repeater-field-control-wrap li:last-child .anecdote-lite-repeater-fields').addClass('wedevs-sortable-active extended');
            $('.anecdote-lite-repeater-field-control-wrap li:last-child .anecdote-lite-repeater-fields').show();

            $('.wedevs-repeater-wrap.wedevs-sortable-active .title-rep-wrap').click(function(){
                $(this).next('.anecdote-lite-repeater-fields').slideToggle().toggleClass('extended');
            }); 

            $('.anecdote-lite-repeater-field-control-wrap li:last-child .wedevs-header-title').text('Social Profile');
    ...
    $("#customize-theme-controls").on("click", ".anecdote-lite-repeater-field-remove",function(){
    ...
    $('#customize-theme-controls').on('click', '.anecdote-lite-repeater-field-close', function(){
        $(this).closest('.anecdote-lite-repeater-fields').slideUp();
        $(this).closest('.wedevs-repeater-wrap').toggleClass('expanded');
    });

    /*Drag and drop to change order*/
    $(".anecdote-lite-repeater-field-control-wrap").sortable({
        axis: 'y',
