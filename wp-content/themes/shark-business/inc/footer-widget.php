...
class Shark_Business_Footer_Widgets{
	 ...
	 * @var int
	 */
	protected $theme_prefix = 'shark_business';
	...
	function init() {
		...
		if ( $this->active_widgets > 0 ) {
			...
			add_action( $this->theme_prefix . '_footer_start_action', array( $this, 'add_footer_widgets' ), 20 );
		}
	...
	function footer_widgets_init() {
		...
		for ( $i = 1; $i <= $this->max_widgets; $i++ ) {
			register_sidebar( array(
				'name'          => sprintf( __( 'Footer Widget %d', 'shark-business' ), $i ),
	...
	function get_footer_widgets_content( $args ) {
		...
		$args = wp_parse_args( (array) $args, array(
			'container'       => 'div',
			'container_class' => $this->custom_footer_widget_class( 'footer-widgets-area' ),
			'container_style' => '',
			'container_id'    => '',
			'wrap_class'      => 'column-wrapper',
			'before'          => '',
			'after'           => '',
		) );
	...
	function custom_footer_widget_class( $input ) {
		...
		$input .= ' column-' . $footer_widgets_number;
...
// Initialize.
$shark_business_footer_widgets = new Shark_Business_Footer_Widgets();
