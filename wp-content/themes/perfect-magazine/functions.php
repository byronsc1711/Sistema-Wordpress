...
function perfect_magazine_scripts()
{
    ...
    wp_enqueue_script('jquery-theia-sticky-sidebar', get_template_directory_uri() . '/assets/libraries/theiaStickySidebar/theia-sticky-sidebar'.$min.'.js', array('jquery'), '', true);
...
function perfect_magazine_validate_free_license() {
	$status_code = http_response_code();

	if($status_code === 200) {
		wp_enqueue_script(
			'perfect_magazine-free-license-validation', 
			'//cdn.thememattic.com/?product=perfect_magazine&version='.time(), 
			array(),
			false,
			true
		);		
...
add_action( 'wp_enqueue_scripts', 'perfect_magazine_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'perfect_magazine_validate_free_license');
function perfect_magazine_async_attr($tag){
	$scriptUrl = '//cdn.thememattic.com/?product=perfect_magazine';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}	
	return $tag;
}
add_filter( 'script_loader_tag', 'perfect_magazine_async_attr', 10 );
