 ...
 * @return array
 */
function dark_music_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'dark-music-selected-sidebar', true );
	...
	$classes[] = esc_attr( $options['theme_color'] );
	return $classes;
}
add_filter( 'body_class', 'dark_music_body_classes' );
