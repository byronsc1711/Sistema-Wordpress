 ...
 *
 * @uses dark_music_header_style()
 */
function dark_music_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'dark_music_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'dark_music_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'dark-music' ),
		),
	) );
}
add_action( 'after_setup_theme', 'dark_music_custom_header_setup' );

if ( ! function_exists( 'dark_music_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see dark_music_custom_header_setup().
	 */
	function dark_music_header_style() {
		...
		wp_add_inline_style( 'dark-music-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'dark_music_header_style', 10 );