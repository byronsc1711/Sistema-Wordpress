...
if ( ! function_exists( 'citadelaTheme_post_locations_list' ) ) :
	/**
	 * Prints HTML with meta information for categories
	 */
	function citadelaTheme_post_locations_list( $separator = '' ) {
		if ( 'post' === get_post_type() ) {
			global $post;
	        ...
	        if ( $locations ) {
				...
				foreach ($locations as $loc) {
	            	array_push(
	            		$loc_links, 
	            		'<a href="' . esc_url( add_query_arg( array( 'ctdl' => 'true', 'post_type' => 'post', 's' => '', 'category' => '', 'location' => $loc->slug ), get_home_url() ) ) . '" rel="location tag">' . $loc->name . '</a>'
	            	);
